...
namespace Ren
{
    ...
    public:
        ...
        Framebuffer& AddAttachment(Ref<FramebufferAttachment> attachment);
        ...
        static Ref<Framebuffer> CreateBasicFramebuffer(unsigned int width, unsigned int height);
        /// Basic framebuffer with one Texture2D color attachment. No depth or stencil buffer is created.
        static Ref<Framebuffer> Create2DBasicFramebuffer(unsigned int width, unsigned int height);
